...
REM Build script for zerobounce using Nuitka
...
set "ENTRY=zerobounce\__main__.py"
set "OUTDIR=build"
set "APPNAME=zerobounce"
for %%I in ("%ENTRY%") do set "ENTRY_BASENAME=%%~nI"
set "ISS_FILE=installer\zerobounce.iss"
set "ICON_PNG=zerobounce\assets\logo.png"
set "ICON_ICO=zerobounce\assets\logo.ico"
...
set "PRODUCT_NAME=ZeroBounce Email Cleaner"
set "FILE_DESCRIPTION=Email cleaning and ZeroBounce result processing tool"
...
set "CERT_SUBJECT=ZeroBounce Self-Signed"
...
if exist "%ICON_PNG%" (
    set "ICON_FLAGS=!ICON_FLAGS! --include-data-files=%ICON_PNG%=zerobounce/assets/logo.png"
)

REM Include config file(s) needed at runtime
set "ICON_FLAGS=!ICON_FLAGS! --include-data-files=cleancolumns.json=cleancolumns.json"
...
if /I "%BUILD_MODE%"=="standalone" (
    if /I "%BUILD_INSTALLER%"=="1" (
        if not exist "%ISS_FILE%" (
            echo [WARN] Inno Setup script not found: %ISS_FILE%
            echo [WARN] Skipping installer generation.
            echo [WARN] To create one, see: https://jrsoftware.org/isinfo.php
            echo [WARN] Set BUILD_INSTALLER=0 to suppress this warning.
            goto :skip_installer
        )
    ...
    :skip_installer
    if /I "%BUILD_INSTALLER%"=="1" if exist "%SETUP_EXE%" echo [INFO] Installer ready: %SETUP_EXE%
)

echo [SUCCESS] Build complete.
echo          Executable file:   %EXE_PATH%
if /I "%BUILD_MODE%"=="standalone" if /I "%BUILD_INSTALLER%"=="1" if exist "%SETUP_EXE%" echo          Installer file:    %SETUP_EXE%
exit /b 0
